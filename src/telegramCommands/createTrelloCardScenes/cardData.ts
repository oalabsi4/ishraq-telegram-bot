...
export const dataFromTrello = {
  async getBoardMembers() {
    if (
      !createCardCache.boardMembers.data ||
      Date.now() - createCardCache.boardMembers.time > cacheClearTime ||
      createCardCache.boardMembers.id !== fetchData.BoardId
    ) {
      createCardCache.boardMembers.data =
        fetchData.BoardId.length > 3 ? await getBoardMembers(fetchData.BoardId) : [{ name: 'No members', id: 'No members' }];
